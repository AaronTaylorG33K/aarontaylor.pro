...
const TheDrip = ({ scrollYProgress }: { scrollYProgress?: any }) => {
  ...
  const theDripAnimation = {
    vector: useTransform(scrollYProgress, [0.9, 1], [n6, n7], {
      ease: cB,
    }),
    scaleY: useSpring(
      useTransform(
        scrollYProgress,
        [0, 0.9, 1],
        [isMobile ? 0.5 : 0.5, isMobile ? 1.5 : 1.5, 0],
    ...
    filter: useTransform(
      scrollYProgress,
      [0, isMobile ? 0.6 : 0.5, isMobile ? 1 : 0.75, isMobile ? 1 : 1],
      [
  ...
  const { borderTopWidth, scaleY, vector, filter } = theDripAnimation;
  return (
    <motion.div
      id="theDrip"
      className={`w-full fixed pointer-events-none  z-40 border-white`}
      style={{ borderTopWidth }}
    >
      <motion.svg
        className=" origin-top border-top border-white "
        viewBox={`0 0 ${isMobile ? 600 : 900} 500`}
        style={{ scaleY }}
      >
        <motion.path
