...
const BlobbyHill = () => {
    ...
    const scale = useTransform(scrollYProgress, [0, 1], [0.5, isMobile ? 1.5: 1],{
        ease: cubicBezier(0.17, 0.67, 0.83, 0.67),
      });
    const rotate = useTransform(scrollYProgress, [0, 1], ['0deg','0deg'],{
        ease: cubicBezier(0.17, 0.67, 0.83, 0.67),
      });
    const x2 =  useTransform(scrollYProgress, [0.4,0.7], ['-25%', '-25%'],{
        ease: cubicBezier(0.17, 0.67, 0.83, 0.67),
      });
    const y2 = useTransform(scrollYProgress, [0.4, 0.7], ['50%', '20%'],{
        ease: cubicBezier(0.17, 0.67, 0.83, 0.67),
      });
    const scale2 = useTransform(scrollYProgress, [0.5, 0.7], [0.5, isMobile? 1.5 : 1],{
        ease: cubicBezier(0.17, 0.67, 0.83, 0.67),
      });
    
    return (
    <div className="w-full h-screen absolute bottom-0 left-0 right-0 z-[11] ">
      <motion.div className="absolute right-0 top-0 w-full" style={{ x, y, rotate, scale }} >
        <svg
          id="Isolation_Mode"
      ...
      <motion.div className="absolute bottom-0 left-0 -translate-x-1/4 translate-y-1/4 w-full" style={{ x:x2, y:y2, scale:scale2}} >
        <svg
          id="Isolation_Mode"
          data-name="Isolation Mode"
          xmlns="http://www.w3.org/2000/svg"
          ...
          <defs>
            <style>
              {`
                ...
                fill: url(#grad1);
                stroke-width: 0px;
                animation: blobAnimation2 10s infinite;
                transform-origin: center;
                }
